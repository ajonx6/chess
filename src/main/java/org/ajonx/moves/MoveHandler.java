...
public class MoveHandler {
	...
	public MoveState makeTemporaryMove(Move move) {
		...
		if (isCastle(move.sfile, move.efile, startPiece)) {
			...
			int rookStartFile = left ? 0 : ChessApp.GRID_SIZE - 1;
	...
	public void makeMove(int startFile, int startRank, int endFile, int endRank, int heldPiece) {
		...
		if (isPromotion(endRank, heldPiece)) {
			...
			if (isEnPassant(endFile, endRank, heldPiece)) {
				...
				board.set(endFile + (left ? 1 : -1), endRank, board.get(left ? 0 : ChessApp.GRID_SIZE - 1, endRank));
				board.set(left ? 0 : ChessApp.GRID_SIZE - 1, endRank, Piece.INVALID);
	...
	public void updateEnPassantTarget(int file, int startRank, int endRank, int piece) {
		if (Piece.isType(piece, Piece.PAWN) &&
			...
			board.enPassantTarget = file + ((startRank + endRank) / 2) * ChessApp.GRID_SIZE;
		} else {
	...
	public void handleTakingRooks(int file, int rank) {
		...
		if (Piece.isColor(piece, Piece.WHITE)) {
			...
			else if (file == ChessApp.GRID_SIZE - 1 && rank == 0) board.whiteRKMove = true;
		} else {
			if (file == 0 && rank == ChessApp.GRID_SIZE - 1) board.blackRQMove = true;
			else if (file == ChessApp.GRID_SIZE - 1 && rank == ChessApp.GRID_SIZE - 1) board.blackRKMove = true;
		}
	}
	...
	public void handlePieceFlags(int startFile, int startRank, int piece) {
		if (Piece.isColor(piece, Piece.WHITE)) {
			if (Piece.isType(piece, Piece.KING)) board.whiteKingMove = true;
				...
				else if (startFile == ChessApp.GRID_SIZE - 1 && startRank == 0) board.whiteRKMove = true;
			}
		} else {
			if (Piece.isType(piece, Piece.KING)) board.blackKingMove = true;
			else if (Piece.isType(piece, Piece.ROOK)) {
				if (startFile == 0 && startRank == ChessApp.GRID_SIZE - 1) board.blackRQMove = true;
				else if (startFile == ChessApp.GRID_SIZE - 1 && startRank == ChessApp.GRID_SIZE - 1)
